"""test_subspace_1B.py
Topic 1B - RMSE & PoR vs snapshot count T.
Read ../data/C5/C5_SnapshotScan.h5 and write ../data/C5/C5_SubspaceNet.h5.
"""
...
OUT_PATH = '../data/C5/C5_SubspaceNet.h5'
K = 2
...
def parse_group(group_name: str) -> float:
    match = re.search(r'T_(\d+)', group_name)
...
with h5py.File(DATA_PATH, 'r') as f:
    ...
    for grp in groups:
        ...
        print(f'T={x_value:g}: RMSE={rmse:.3f} deg, PoR={por:.1f}%')
...
with h5py.File(OUT_PATH, 'w') as fout:
    ...
    fout.create_dataset('T_vec', data=np.array(x_vals, dtype=np.float32)[order])
