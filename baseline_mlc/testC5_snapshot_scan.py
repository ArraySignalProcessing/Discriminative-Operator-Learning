"""
test_cnn_1B.py
Topic 1B: RMSE and PoR versus snapshot count T.
Reads ../data/C5/C5_SnapshotScan.h5 and writes ../data/C5/C5_CNN.h5.
"""
# Unified MATLAB benchmark uses SNR=0 dB for Topic 1B.
...
def plot_all_predictions(t_list, gt_list, est_list) -> None:
    """Save all T values in one figure with 3-column layout."""
    ...
    for i, (t, gt, est) in enumerate(zip(t_list, gt_list, est_list)):
        ax = axes[i]
    ...
    plt.tight_layout()
    save_path = os.path.join(plot_dir, 'C5_prediction.png')
...
with h5py.File(DATA_PATH, 'r') as f:
    ...
    groups = sorted(groups, key=lambda x: int(re.search(r'T_(\d+)', x).group(1)) if re.search(r'T_(\d+)', x) else 0)
    print(f'Found {len(groups)} groups: {groups}\n')
    T_vals, rmse_vals, por_vals = [], [], []
    plot_ts = []
    plot_gts = []
    plot_ests = []

    for grp in groups:
        match = re.search(r'T_(\d+)', grp)
        T = int(match.group(1)) if match else float('nan')
        ...
        por_vals.append(por)
        print(f'T={T:4d}: RMSE={rmse:.3f} deg, PoR={por:.1f}%')

order = np.argsort(T_vals)
plot_all_predictions(plot_ts, plot_gts, plot_ests)
with h5py.File(OUT_PATH, 'w') as fout:
    fout.create_dataset('T_vec', data=np.array(T_vals)[order])
