"""
test_cnn_1E.py
Topic 1E: array-imperfection rho scan, RMSE and PoR vs rho.
Reads ../data/C3/C3_ArrayMismatch.h5 and writes
../data/C3/C3_CNN.h5.
"""
...
def plot_all_predictions(rho_list, gt_list, est_list) -> None:
    ...
    plt.tight_layout()
    save_path = os.path.join(plot_dir, 'C3_prediction.png')
...
with h5py.File(DATA_PATH, 'r') as f:
    ...
    groups = sorted(groups, key=lambda x: float(re.search(r'Rho_([\d.]+)', x).group(1)) if re.search(r'Rho_([\d.]+)', x) else 0)
    ...
    for grp in groups:
        match = re.search(r'Rho_(\d+\.?\d*)', grp)
        rho = float(match.group(1)) if match else float('nan')
        ...
        por_vals.append(por)
        print(f'rho={rho:.2f}: RMSE={rmse:.3f} deg, PoR@{THRESHOLD:.1f}deg={por:.1f}%')

order = np.argsort(rho_vals)
plot_all_predictions(plot_rhos, plot_gts, plot_ests)
with h5py.File(OUT_PATH, 'w') as fout:
    fout.create_dataset('rho_all', data=np.array(rho_vals)[order])
    ...
    fout.create_dataset('resolution_threshold_deg', data=THRESHOLD)
