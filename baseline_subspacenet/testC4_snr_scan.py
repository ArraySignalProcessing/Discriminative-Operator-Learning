"""test_subspace_1A.py
Topic 1A - RMSE & PoR vs SNR.
Read ../data/C4/C4_SnrScan.h5 and write ../data/C4/C4_SubspaceNet.h5.
"""
...
OUT_PATH = '../data/C4/C4_SubspaceNet.h5'
K = 2
THRESHOLD = 1.0
...
def plot_all_predictions(x_list, gt_list, est_list) -> None:
    ...
    for i, (x_value, gt, est) in enumerate(zip(x_list, gt_list, est_list)):
        ...
        ax.set_title(f'SNR={x_value:g}dB')
...
with h5py.File(DATA_PATH, 'r') as f:
    ...
    for grp in groups:
        ...
        print(f'SNR={x_value:g} dB: RMSE={rmse:.3f} deg, PoR={por:.1f}%')
...
with h5py.File(OUT_PATH, 'w') as fout:
    ...
    fout.create_dataset('SNR_vec', data=np.array(x_vals, dtype=np.float32)[order])
