% plotC5_snapshot_scan.m
% 绘制 Topic 1B：RMSE vs T
% 自动检测所有可用结果文件。
% 作者: D
...
% ================= 加载传统基线 =================
baseline_file = fullfile(data_dir, 'C5_Baselines.mat');
if ~exist(baseline_file, 'file')
    error('找不到基线结果: %s', baseline_file);
end
load(baseline_file, 'T_vec', 'RMSE', 'PoR');

% 提取基线
MUSIC_rmse    = RMSE.MUSIC;
RMUSIC_rmse   = RMSE.RootMUSIC;
ESPRIT_rmse   = RMSE.ESPRIT;
ShrinkageMUSIC_rmse = get_struct_vector(RMSE, 'ShrinkageMUSIC', size(T_vec));
...
ShrinkageMUSIC_por = get_struct_vector(PoR, 'ShrinkageMUSIC', size(T_vec)) * 100;
l1SVD_por     = PoR.l1SVD * 100;
UnESPRIT_por  = PoR.UnESPRIT * 100;

% ================= 尝试加载 CNN 结果 =================
cnn_file = fullfile(data_dir, 'C5_CNN.h5');
CNN_rmse = nan(size(T_vec));
CNN_por  = nan(size(T_vec));
...
% ================= 尝试加载 Ours 结果 =================
ours_file = fullfile(data_dir, 'C5_Ours.h5');
Ours_rmse = nan(size(T_vec));
Ours_por  = nan(size(T_vec));
...
% ================= 尝试加载 SubspaceNet 结果 =================
root_ours_file = fullfile(data_dir, 'C5_RootOurs.h5');
RootOurs_rmse = nan(size(T_vec));
RootOurs_por  = nan(size(T_vec));
...
subspace_file = fullfile(data_dir, 'C5_SubspaceNet.h5');
SubspaceNet_rmse = nan(size(T_vec));
SubspaceNet_por  = nan(size(T_vec));
...
% 输出表格
Tab_RMSE = array2table([T_vec(:), methodRMSE], ...
    'VariableNames', ['T', methodNames]);
fprintf('=== Topic 1B RMSE 表格 ===\n');
disp(Tab_RMSE);
% PoR is intentionally not displayed for Topic 1B; C.2 uses RMSE only.
...
xlabel('Number of snapshots T', 'Interpreter', 'latex');
title('RMSE versus snapshots', 'Interpreter', 'latex', 'FontWeight', 'normal', 'FontSize', 10);
...
export_ieee_figure(fig, 'plotC5_snapshot_scan');
